...
public class RoomAndClockTests
{
    ...
    {
        var dir = Path.Combine(Path.GetTempPath(), "auralis-" + Guid.NewGuid().ToString("n"));
...
{
    [Fact]
    public void AatpFrameRoundTripsEncryptedPayloadWithEpoch()
    {
        var key = System.Security.Cryptography.RandomNumberGenerator.GetBytes(32);
        var pcm = new byte[] { 1, 2, 3, 4, 5, 6, 7, 8 };
        var frame = AatpFrame.Encode(new AatpAudio(12000, 96000, 24, 2, false, 7, pcm), key);

        Assert.True(AatpFrame.TryDecode(frame, key, out var decoded));
        ...
        // 키가 없으면 페이로드를 열 수 없다.
        Assert.False(AatpFrame.TryDecode(frame, null, out _));
    }

    [Fact]
    public void DsdFramesKeepTheirNativeFlag()
    {
        var frame = AatpFrame.Encode(new AatpAudio(0, 2822400, 1, 2, true, 0, [0xAA, 0x55]), null);
        Assert.True(AatpFrame.TryDecode(frame, null, out var decoded));
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), "auralis-" + Guid.NewGuid().ToString("n") + ".wav");
