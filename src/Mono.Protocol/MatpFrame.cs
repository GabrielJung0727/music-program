...
/// <summary>
/// MATP 데이터 플레인 한 청크. 소셜 메시지는 이 경로에 실리지 않는다.
/// </summary>
public readonly record struct MatpAudio(
    long PtsMs,
    int SampleRate,
    int BitDepth,
    int Channels,
...
public static class MatpFrame
{
    public static readonly byte[] Magic = "MATP"u8.ToArray();

    private const int HeaderSize = 44;
    private const int FlagEncrypted = 1;
    private const int FlagDsd = 2;

    public static byte[] Encode(MatpAudio audio, byte[]? aesKey)
    {
    ...
    public static bool TryDecode(byte[] frame, byte[]? aesKey, out MatpAudio audio)
    {
        audio = default;
        ...
        audio = new MatpAudio(pts, rate, depth, channels, (flags & FlagDsd) != 0, epoch, payload);
        return true;
    }
