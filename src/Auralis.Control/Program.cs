...
// Auralis Control — 명령과 시각화만. 오디오 콜백을 돌리지 않는다.
...
_ = Task.Run(async () =>
{
    ...
    {
        var msg = JsonSerializer.Deserialize<AuralisMessage>(line, LineFraming.JsonOptions);
...
while (true)
{
    ...
    try
    {
        ...
        AuralisMessage? m = p[0].ToLowerInvariant() switch
        {
            // 룸
...
async Task Send(AuralisMessage message) => await stream.WriteAsync(LineFraming.Encode(message));

static void PrintRoom(AuralisMessage msg)
{
    try
    {
