...
public sealed class LoungeHub : Hub
{
    ...
    public async Task Send(AuralisMessage message)
    {
        ...
        if (message.Type == MessageTypes.Hello)
        {
            ...
            Context.Items["peer"] = peer;
            Register(peer, Context.ConnectionId);
            await Clients.Caller.SendAsync("msg", new AuralisMessage { Type = MessageTypes.Welcome, PeerId = peer, Ok = true });
            await Clients.Caller.SendAsync("msg", _commands.CatalogMessage());
            var rooms = _commands.Execute(peer, new AuralisMessage { Type = MessageTypes.ListRooms }, name);
        ...
        {
            await Clients.Caller.SendAsync("msg", new AuralisMessage
            {
