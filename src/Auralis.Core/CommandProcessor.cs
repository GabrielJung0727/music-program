...
public sealed class CommandProcessor
{
    ...
    public CommandResult Execute(string peerId, AuralisMessage msg, string? displayName)
    {
        switch (msg.Type)
        {
            // ── 룸 ──────────────────────────────────────────────
            ...
            case MessageTypes.ListRooms:
                return Direct(new AuralisMessage
                {
            ...
            case MessageTypes.ScanLibrary:
            {
                ...
                return new CommandResult(null, new AuralisMessage
                {
            ...
            case MessageTypes.CreatePlaylist:
            {
                ...
                if (ids.Count == 0 && msg.ArchiveId is not null)
                {
                    ...
                    return Direct(new AuralisMessage { Type = MessageTypes.CreatePlaylist, Ok = true, PlaylistId = created.Id, Body = created.Title });
                }
                ...
                return Direct(new AuralisMessage { Type = MessageTypes.CreatePlaylist, Ok = true, PlaylistId = playlist.Id, Body = playlist.Title });
            }
            ...
            case MessageTypes.Redeem:
            {
                ...
                return session is null
                    ...
                    : Direct(new AuralisMessage { Type = MessageTypes.Redeem, Ok = true, Token = session, PeerId = issuedBy });
            }
    ...
    public AuralisMessage CatalogMessage() => new()
    {
    ...
    private static CommandResult From((ListeningRoom? Room, string? Error) r)
        => r.Error is not null && r.Room is null
            ? Fail(r.Error)
            : r.Error is not null
                ? new CommandResult(r.Room, new AuralisMessage { Type = MessageTypes.Error, Ok = false, Error = r.Error })
                : Broadcast(r.Room!);

    private static CommandResult Broadcast(ListeningRoom room) => new(room, null);
    private static CommandResult Direct(AuralisMessage msg) => new(null, msg);
    private static CommandResult Fail(string error) => new(null, new AuralisMessage { Type = MessageTypes.Error, Ok = false, Error = error });
    ...
    private string NeedRoom(string peerId, AuralisMessage msg)
    {
...
public readonly record struct CommandResult(
    ...
    AuralisMessage? Direct,
    AuralisMessage? BroadcastAll = null);
