...
/// <summary>
/// AATP 데이터 플레인 송신기. 재생 헤드보다 앞선 구간을 미리 보내고(lookahead),
        ...
        while (state.CursorMs < now + LookaheadMs && (duration <= 0 || state.CursorMs < duration))
        {
            ...
            foreach (var peerId in listeners)
            {
                ...
                if (needsDigitalVolume)
                {
                    ...
                    frame = AatpFrame.Encode(
                        new AatpAudio(basePts, rate, depth, format.Channels, format.IsDsd, room.ResyncEpoch, attenuated),
                        room.FanOutKey);
                }
                else
                {
                    shared ??= AatpFrame.Encode(
                        new AatpAudio(basePts, rate, depth, format.Channels, format.IsDsd, room.ResyncEpoch, payload),
                        room.FanOutKey);
                    frame = shared;
                }

                var line = LineFraming.Encode(new AuralisMessage
                {
                    Type = MessageTypes.AatpAudio,
                    RoomId = room.Id,
                    MediaTimeMs = basePts,
                    SampleRate = rate,
                    BitDepth = depth,
